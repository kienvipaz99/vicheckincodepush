...
function PanResponderAdapter(_ref) {
  ...
  const jumpToIndex = React.useCallback(
    function (index) {
      let animate =
        arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : animationEnabled;
  ...
  const translateX = _reactNative.Animated.multiply(
    panX.interpolate({
  ...
  const position = React.useMemo(
    () => (layout.width ? _reactNative.Animated.divide(panX, -layout.width) : null),
    [layout.width, panX],
  );
  return children({
