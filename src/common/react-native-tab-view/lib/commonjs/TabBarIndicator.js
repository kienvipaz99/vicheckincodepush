...
function TabBarIndicator(_ref) {
  let {getTabWidth, layout, navigationState, position, width, gap, style} = _ref;
  const isIndicatorShown = React.useRef(false);
  const isWidthDynamic = width === 'auto';
  const opacity = (0, _useAnimatedValue.default)(isWidthDynamic ? 0 : 1);
  const hasMeasuredTabWidths =
    Boolean(layout.width) && navigationState.routes.every((_, i) => getTabWidth(i));
  React.useEffect(() => {
    const fadeInIndicator = () => {
      if (
        !isIndicatorShown.current &&
        isWidthDynamic && // We should fade-in the indicator when we have widths for all the tab items
