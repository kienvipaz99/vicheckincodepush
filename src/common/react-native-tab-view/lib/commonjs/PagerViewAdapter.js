...
const AnimatedViewPager = _reactNative.Animated.createAnimatedComponent(
  _reactNativePagerView.default,
);
...
function PagerViewAdapter(_ref) {
  ...
  const jumpTo = React.useCallback(
    key => {
      ...
      if (animationEnabled) {
        ...
        (_pagerRef$current = pagerRef.current) === null || _pagerRef$current === void 0
          ? void 0
          : _pagerRef$current.setPage(index);
      } else {
        var _pagerRef$current2;

        (_pagerRef$current2 = pagerRef.current) === null || _pagerRef$current2 === void 0
          ? void 0
          : _pagerRef$current2.setPageWithoutAnimation(index);
  ...
  React.useEffect(() => {
    ...
    if (indexRef.current !== index) {
      if (animationEnabled) {
        ...
        (_pagerRef$current3 = pagerRef.current) === null || _pagerRef$current3 === void 0
          ? void 0
          : _pagerRef$current3.setPage(index);
      } else {
        var _pagerRef$current4;

        (_pagerRef$current4 = pagerRef.current) === null || _pagerRef$current4 === void 0
          ? void 0
          : _pagerRef$current4.setPageWithoutAnimation(index);
        position.setValue(index);
      }
    }
  ...
  const memoizedPosition = React.useMemo(
    () => _reactNative.Animated.add(position, offset),
    [offset, position],
  );
  return children({
